...
# Perform hyper-parameter tuning for Spark's random forest of hoeffding trees
  ...
  do
   ...
   do
    ...
    do
     ...
     do
       out_filename="spark_rf_c${splitconf}_t${tiethre}_g${grace}_s${splitcrit}_d${depth}_e${treesize}"
       echo ${out_filename}

       ${run_spark} "${batchInterval} EvaluatePrequential" \
         " -l (org.apache.spark.streamdm.classifiers.meta.RandomForest" \
         " -l (org.apache.spark.streamdm.classifiers.trees.HoeffdingTree -c ${splitconf} -t ${tiethre} -g ${grace} -h ${depth}) -s ${treesize})" \
      ...
      sed -i '/INFO RandomForest: /d' "${output_dir}/${out_filename}.log"
     done
    done
   done
  done
