...
def writeStatsToFile(outputDir, confs, hps, results, accuracies, jobTimes, taskTimes):
    ...
    for i,row in enumerate(overall_best_SP_per_HP):
        fp.write('{:60}|{:>20}|{:>20.2f}\n'.format(hps[i], row + 1, overall_best_SP_per_HP[i]))
    ...
    for i, row in enumerate(overall_best_HP_per_SP):
        fp.write('{:20}|{:>60}|{:>20.2f}\n'.format(i + 1, hps[row], overall_best_HP_per_SP[i]))
...
def writeStatsToCSVFile(outputDir, confs, hps, results, accuracies, f1Scores, jobTimes, taskTimes):
    accuraciesLastHalf = np.array(results)
    normalizedAccuracies = normalize(accuraciesLastHalf, np.min(accuraciesLastHalf), np.max(accuraciesLastHalf))

    overallAccuracies = np.array(accuracies)
    ...
    with open(os.path.join(outputDir, "results.csv"), 'w', newline='') as csvfile:
        ...
        writer.writerow(["l", "o", "r", "p", "Configuration", "Accuracy (%)", "Normalized Accuracy (%)", "Overall Accuracy (%)", "Normalized Overall Accuracy (%)", "F1-Score (%)", "Normalized F1-Score (%)", "Average Job Time (sec)", "Normalized Average Job Time (%)", "Average Task Time (sec)", "Normalized Average Task Time (%)", "Average Normalized Time Joined (%)"])
        for i, r in enumerate(results):
            for j, c in enumerate(r):
                writer.writerow(hpsValues(hps[i]) + [j + 1, accuraciesLastHalf[i][j], normalizedAccuracies[i][j], overallAccuracies[i][j], normalizedOverallAccuracies[i][j], f1Scores[i][j], normalizedF1Scores[i][j], jobTimes[i][j], normalizedJobtimes[i][j], taskTimes[i][j], normalizedTaskTimes[i][j], (normalizedJobtimes[i][j] + normalizedTaskTimes[i][j]) / 2]) 
...
def main(args):
    ...
    writeStatsToCSVFile(outputDir, confs, hps, accuraciesLastHalf, accuracies, f1Scores, jobTimes, taskTimes)
